      ...
      <div className="flex gap-4 overflow-x-auto pb-2">
        {columns.map((col, colIndex) => {
          ...
          return (
            <motion.div
              key={col.key}
              className="flex-shrink-0 w-80 flex flex-col rounded-xl bg-gray-100/80 overflow-hidden"
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.2, delay: colIndex * 0.05 }}
            >
              <div className="px-4 py-3 bg-gray-200/90 rounded-t-xl">
...
function TaskCard({ task, index }: { task: TaskCard; index: number }) {
  const tagClass = task.status === "done" ? "bg-green-100 text-green-700" : typeStyles[task.type];
  return (
    <motion.div
    ...
    >
      ...
      <p className="mt-2 text-sm text-gray-900 leading-snug">{task.title}</p>
